...
/// <-- ОБНОВЛЕНО: Создание иллюзии случайного чередования призов -->
function generateRouletteItems(prizes) {
    ...
    const visualPrizes = [];
    // 1. Устанавливаем соотношение. Например, 2 означает, что "пустышек"
    // будет примерно в 2 раза больше, чем настоящих призов.
    const nothingRatio = 2;

    const realPrizes = prizes.filter(p => p.name !== 'Ничего');
    const nothingPrize = prizes.find(p => p.name === 'Ничего');

    // 2. Наполняем массив реальными призами и "пустышками" в нужной пропорции
    visualPrizes.push(...realPrizes); // Сначала добавляем все реальные призы

    if (nothingPrize) {
        // Добавляем "Ничего" в количестве, в `nothingRatio` раз превышающем число реальных призов
        const nothingCount = realPrizes.length * nothingRatio;
        for (let i = 0; i < nothingCount; i++) {
            visualPrizes.push(nothingPrize);
        }
    }

    // 3. 🔥 ТЩАТЕЛЬНО ПЕРЕМЕШИВАЕМ! Это ключевой шаг для создания иллюзии случайности.
    visualPrizes.sort(() => Math.random() - 0.5);

    // 4. Дублируем получившийся случайный набор для создания длинной ленты
